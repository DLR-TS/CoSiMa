...
TEST_CASE("internalstate is availible") {
	...
	REQUIRE(simulationdata->getInternalState()->bools.at(0) == 0.0);
	REQUIRE(simulationdata->getInternalState()->bools.at(1) == 1000.5);
	simulationdata->getInternalState()->floats.push_back(0.67f);
	simulationdata->getInternalState()->floats.push_back(-7.9f);
	REQUIRE(simulationdata->getInternalState()->bools.at(0) == 0.67f);
	REQUIRE(simulationdata->getInternalState()->bools.at(1) == -7.9f);
	simulationdata->getInternalState()->integers.push_back(10056);
	simulationdata->getInternalState()->integers.push_back(-59574);
	REQUIRE(simulationdata->getInternalState()->bools.at(0) == 10056);
	REQUIRE(simulationdata->getInternalState()->bools.at(1) == -59574);
